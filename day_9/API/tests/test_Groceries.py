...
class TestVegetableList:
    def test_post_success(self):
            response = tester.post('/vegetables', json={"name":"chilli","quantity":5.5})
...
class TestVegetable:
    ...
    def test_put_update(self):
        response = tester.put('/vegetable/{}'.format("chilli"), json={"quantity":15.6})
        status = response.status_code
        assert status == 201
        
    def test_put_insert(self):
        response = tester.put('/vegetable/{}'.format("potato"), json={"quantity":10})
        status = response.status_code
        assert status == 201
